...
exports.updatePost = (req, res, next) => {
  ...
  Post.updateOne({_id: req.body.id, creator: req.userData.userId}, post).then(result => {
    if(result.n > 0) res.status(200).json({message: "Update Successfully!"});
...
exports.getPosts = (req, res, next) => {
  ...
  if(pageSize && currentSize) {
    postQuery
      .skip(pageSize * (currentSize - 1))
      .limit(pageSize);
  }
  postQuery.then(documents => {
    fethedPosts = documents;
    return Post.count();
  }).then(count => {
    res.status(200).json({
...
exports.deletePost = (req, res, next) => {
  Post.deleteOne({_id: req.params.id, creator: req.userData.userId}).then(result => {
    ...
    else res.status(200).json({message: "Not Authorized"});
  });
}